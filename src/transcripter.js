...
const Transcripter = async (url) => {
    let { apiKey, videobody } = await getInnerApiKey(url);
    let transcripts;
    if (apiKey && apiKey.length > 0) {
        transcripts = await getTranscripts(videobody, apiKey)
        return transcripts
    }
}

async function getTranscripts(videobody, key) {
    try {
        const { body } = await p({
            "url": `https://www.youtube.com/youtubei/v1/get_transcript?key=${key}`,
            method: "POST",
            data: generateRequest(videobody.toString()),
            parse: 'json'
        })
        if (body.responseContext) {
            if (!body.actions) {
                throw new Error('Transcript not provided for this video');
            }
...
async function getInnerApiKey(url) {
    let { body: videobody } = await p({ url })
    const apiKey = videobody
        .toString()
        .split('"INNERTUBE_API_KEY":"')[1]
        .split('"')[0];
    return { apiKey, videobody };
}
